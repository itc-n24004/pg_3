...
for quiz_num in range(issue):
    ...
    for question_num in range(len(prefectures)):
        #正答と誤答を抽出する
        correct_answer = capitals[ prefectures[question_num] ] #正解の県庁所在地
        wrong_answers = list(capitals.values()) #県庁所在地だけをリストに抽出
        del wrong_answers[wrong_answers.index(correct_answer)] #正解の県庁所在地だけ除去
        wrong_answers = random.sample(wrong_answers, 3) #不正解の選択肢を3つだけ抽出
        answer_options = wrong_answers + [correct_answer] #正解と不正解の選択肢を合わせる
